...
void OsisParticipant::ProcessAttribute(int key, QString& value)
{
   switch (static_cast<enum OsisElementAttributes>(key))
   {
      case ID:
         Participant_ID = value.toInt();
         break;
      case Status:
         {
            uint Status = getEnumKey("OsisParticipantStatus", value.toLocal8Bit().constData());
            if (Status > Status_MAX)
            {
               qCritical() << "Unknown <Participant> Status attribute: " << value << endl;
               break; // Error report
            }
